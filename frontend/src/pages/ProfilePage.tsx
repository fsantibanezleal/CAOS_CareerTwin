...
function ArtifactStudio({ claims }: { claims: Claim[] }) {
  ...
  const [title, setTitle] = useState(() => t('Evidence-grounded résumé'))
  ...
  return (
    <Panel title={t('Artifact studio')} subtitle={t('Versioned drafts assembled only from confirmed evidence')}>
...
export function ProfilePage() {
  ...
  return (
    <>
      ...
      {tab === 'overview' && <div className="profile-layout"><Panel title={t('Identity and direction')} subtitle={t('User-curated canonical fields')}><ProfileEditor profile={profile.data} /></Panel><SkillsPanel skills={skills.data} claims={claims.data} /><TimelineEditors experiences={experiences.data} education={education.data} /></div>}
