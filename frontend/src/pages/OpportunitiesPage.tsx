...
import { ArrowUpRight, BriefcaseBusiness, Building2, CalendarClock, Check, FileUp, FolderKanban, Globe2, History, LayoutGrid, Link2, List, MapPin, Plus, Radar, Search, Sparkles, Trash2, X } from 'lucide-react'
...
import type { Landscape, Opportunity, OpportunitySnapshot, Requirement, TargetSet } from '../types'
  ...
  return (
    <Panel title="Opportunity intelligence" subtitle={`Version ${opportunity.version} · extraction is editable, never silently canonical`} actions={draft.source_url && <ExternalLink href={draft.source_url}>Source</ExternalLink>}>
      <form className="opportunity-editor" onSubmit={(event) => { event.preventDefault(); update.mutate() }}><div className="form-grid two"><label>Role title<input value={draft.title} onChange={(event) => field('title', event.target.value)} /></label><label>Employer<input value={draft.employer} onChange={(event) => field('employer', event.target.value)} /></label></div><label>Posting text<textarea rows={8} value={draft.description} onChange={(event) => field('description', event.target.value)} /></label><div className="form-grid four"><label>Industry<input value={draft.industry} onChange={(event) => field('industry', event.target.value)} /></label><label>Area<input value={draft.area} onChange={(event) => field('area', event.target.value)} /></label><label>Seniority<input value={draft.seniority} onChange={(event) => field('seniority', event.target.value)} /></label><label>Work mode<select value={draft.remote_mode} onChange={(event) => field('remote_mode', event.target.value)}><option value="unspecified">Unspecified</option><option value="remote">Remote</option><option value="hybrid">Hybrid</option><option value="onsite">On-site</option></select></label></div>
        ...
        <details className="evidence-history"><summary><History /> Immutable revision history ({history.data?.length ?? 0})</summary>{history.data?.map((item) => <div key={item.id}><span className="status-badge">Version {item.version}</span><p><b>{item.snapshot.title}</b> · {item.snapshot.requirements.length} requirements · {new Date(item.created_at).toLocaleString()}</p></div>)}</details>
...
function TargetSetManager({ opportunities }: { opportunities: Opportunity[] }) {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { setName(''); setSelected([]); client.invalidateQueries({ queryKey: ['target-sets'] }) },
  })
  const remove = useMutation({ mutationFn: (id: string) => api(`/api/opportunities/target-sets/${id}`, { method: 'DELETE' }), onSuccess: () => client.invalidateQueries({ queryKey: ['target-sets'] }) })
  const toggle = (id: string) => setSelected((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id])
  return (
    <Panel title="Target portfolios" subtitle="Save named job-search scenarios so global alignment always has an explicit denominator" actions={<span className="status-badge"><FolderKanban /> {targetSets.data?.length ?? 0} sets</span>}>
      <form className="target-set-compose" onSubmit={(event) => { event.preventDefault(); create.mutate() }}>
        <label>Portfolio name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Remote platform leadership" required /></label>
        <fieldset><legend>Include saved opportunities</legend><div className="target-options">{opportunities.map((item) => <label key={item.id}><input type="checkbox" checked={selected.includes(item.id)} onChange={() => toggle(item.id)} /><span><b>{item.title}</b><small>{item.employer || 'Employer unknown'}</small></span></label>)}</div></fieldset>
        <button className="button primary" disabled={!selected.length || create.isPending}><Plus /> Save target portfolio</button>
      </form>
      {(targetSets.error || create.error || remove.error) && <ErrorState error={targetSets.error || create.error || remove.error} />}
      {targetSets.data?.length ? <div className="target-set-list">{targetSets.data.map((item) => <article key={item.id}><div><b>{item.name}</b><small>{item.opportunity_ids.length} role{item.opportunity_ids.length === 1 ? '' : 's'} · explicit scenario</small></div><button className="icon-button" aria-label={`Delete ${item.name}`} onClick={() => remove.mutate(item.id)}><Trash2 /></button></article>)}</div> : <EmptyState title="No target portfolio yet" description="Select the roles you genuinely want to compare as one career-search scenario." />}
    </Panel>
  )
}
