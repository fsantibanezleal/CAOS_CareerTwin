  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <button className="brand-lockup compact" onClick={() => navigate('/')}><span className="brand-mark"><Orbit /></span><span>CareerTwin<small>{t('Career intelligence')}</small></span></button>
        <nav aria-label={t('Primary navigation')}>{nav.map(([path, label, Icon]) => <NavLink key={path} to={path} end={path === '/'}><Icon /><span>{label}</span></NavLink>)}</nav>
        <div className="sidebar-trust"><Shield /><div><b>{t('Private by design')}</b><small>{t('Agents propose. You decide.')}</small></div></div>
        <button className="architecture-trigger" onClick={() => setArchitecture(true)}><Network /><span><b>{t('System architecture')}</b><small>{t('Inspect all six views')}</small></span></button>
      </aside>
      <div className="shell-main">
        <header className="topbar">
          <button className="command-search" onClick={() => setChat(true)}><Search /><span>{t('Search or ask CareerTwin')}</span><kbd>Ctrl K</kbd></button>
          <div className="topbar-actions">
            ...
            <div className="account-menu"><button onClick={() => setAccount(!account)}><span className="avatar">{user.display_name.slice(0, 2).toUpperCase()}</span><span><b>{user.display_name}</b><small>{t(user.is_superuser ? 'Superuser + seeker' : 'Seeker workspace')}</small></span><ChevronDown /></button>{account && <div className="account-popover">{user.is_superuser && <NavLink to="/admin" onClick={() => setAccount(false)}><UserRoundCog /> {t('Account administration')}</NavLink>}<button onClick={() => { setAccount(false); setSecurity(true) }}><KeyRound /> {t('Account security')}</button><button onClick={() => logout.mutate()}><LogOut /> {t('Sign out')}</button></div>}</div>
          </div>
        </header>
        <main id="main-content" className="content">{user.must_change_password && <div className="security-banner">{t('Your account uses a temporary password.')} <button className="text-button" onClick={() => setSecurity(true)}>{t('Change it before adding private documents.')}</button></div>}{children}</main>
      </div>
