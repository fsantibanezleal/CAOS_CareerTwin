...
export function Loading({ label = 'Loading' }: { label?: string }) {
  return <div className="state-message" role="status"><LoaderCircle className="spin" />{label}</div>
}
...
export function Score({ value, label = 'alignment' }: { value?: number; label?: string }) {
  if (value === undefined || value === null) return <span className="score unknown">Insufficient evidence</span>
  const percent = Math.round(value * 100)
  return <span className="score" aria-label={`${percent}% ${label}`}><b>{percent}</b><small>%</small></span>
}
