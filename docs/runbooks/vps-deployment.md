...
3. Start PostgreSQL and malware scanning, run the one-shot Alembic migration, then start app and worker.
4. Mount the same encrypted blob volume into app/worker. Keep PostgreSQL and blob volumes persistent; the worker queue needs no separate volume.
5. Import operator-downloaded pinned ESCO/O*NET datasets only when needed. Do not generate local embeddings or commit dataset archives.
