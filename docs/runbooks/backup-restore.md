...
Run `scripts/backup.ps1` or `scripts/backup.sh`. The default path `backups/private/` is ignored. Encrypt and copy the set off-host; record a checksum and retention class without exposing names from user data.
