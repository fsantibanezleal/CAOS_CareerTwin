...
`0.2.4`: complete self-hosted career-research workflows with real private model/document services, encrypted sources, hybrid taxonomies, career artifacts, consent-bound personal connectors, distroless-compatible backup operations, fail-fast AES-256 key validation, rate-aware production verification, and portable PostgreSQL restore checks. See `CHANGELOG.md` for release evidence and GitHub issues for tracked changes.
