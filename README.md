...
Release `0.10.0`. See [`CHANGELOG.md`](CHANGELOG.md).
...
Version `0.5.10` carries the decision-grade guided experience and accessible dynamic-visualization
...
mobile content from bottom-navigation overlap, and provides a localized keyboard skip link whose
target receives keyboard focus across boot, login, and authenticated surfaces. The saved account
language also takes control immediately when an anonymous session becomes authenticated, while
Sigma network surfaces and their canvas-rendered labels, relationships, filters, and hover states
retain readable active light or dark workbench tokens, with Unicode-safe, viewport-aware overview
labels that avoid dense cluster collisions and prioritize decision-significant nodes while full
entity text remains inspectable. The
